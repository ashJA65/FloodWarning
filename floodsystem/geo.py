...
def stations_by_distance(stations, p):
    ...
    SBDoutputList =[] #will append results into this
    distance = 0 #working distance variable

    for i in stations:
        distance = haversine(i.coord,p) 
        SBDoutputList.append((i,distance))
    
    SBDoutputList.sort(key=lambda y: y[1]) # should sort list by the second item of the tuple
    #https://pythonguides.com/python-sort-list-of-tuples/

    return SBDoutputList
